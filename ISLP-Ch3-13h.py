...
# Generate a vector 'eps' with 100 observations from N(0, 0.05) distribution (decreased variance)
eps = np.random.normal(0, 0.05, 100)  # Decreased variance here
...
plt.xlabel('x')
plt.ylabel('y')
plt.title('Scatterplot with Regression Lines (Less Noise)')
plt.legend()
...
print("R-squared value for the linear regression model (less noise):", r_squared)
